...
11. `supabase/migrations/0011_signup_cannot_pick_a_role.sql` — **signing up can no longer choose its own role (security)**
12. `supabase/migrations/0012_proof_storage.sql` — the private bucket proof photos live in
...
| Signing up cannot make you staff | The signup trigger ignores any `role` or `org_id` in the account's metadata, because that metadata belongs to whoever called the public signup endpoint. Everyone who signs up is a client with no studio; role and org are set afterwards by the functions that have already authorized their caller. Verified against real Postgres both ways: on the old trigger a forged signup really did become an `org_admin` inside her studio and could read a staff record; on the new one it gets `owner`, no org, and sees no cleaner, client, property or job. |
| Starting your own book | A housekeeper who finds the app can open their own studio — a new organization with one person in it — instead of being someone's client or waiting for an invitation. `become-contractor` requires the caller's own token and can only ever create a NEW organization, so it cannot be aimed at an existing one, and it refuses anyone already in a studio rather than moving them. If Supabase holds the account back for email confirmation, the studio is created on their first real sign-in instead. |
| Photo proof | The Kee Method's photo moments are real. The file goes from the phone straight into a **private** bucket whose policy accepts a key only inside the uploader's own studio folder; `attach-photo` then records it against the step. Two fields are never the browser's to choose: `marketing_consent` is always false, and whether a shot is a 'before' comes from the checklist phase, not from whoever is holding the phone. Viewing is a link signed for five minutes by `photo-url` after checking the asker is the studio's staff or the home's owner — so "never publicly linkable" holds even if a link leaks. A clean cannot be closed with a photo moment still missing its picture, because the owner's report says the clean was documented. Verified against real Postgres: a contractor cannot upload into another studio's folder, cannot read its objects, and a client can neither read the bucket directly nor file proof of their own clean. |
| Inventory → Instacart | Each home holds a par level per item and a count of what's actually in the cupboard; the gap **is** the reorder list, so nobody types a shopping list. Counting a line writes back immediately. "Send to Instacart" opens the **Instacart app** — instacart.com publishes an apple-app-site-association, so an ordinary `https://` link is a universal link that iOS and Android hand to the installed app and that falls back to the web when it isn't (an `instacart://` scheme would fail with nothing to show). Linens are `supplier_only` and are deliberately kept out of the grocery link. Each reorder is recorded as a `reorders` row before the handoff, so there's a history rather than a link somebody tapped once. **Nothing is ordered or charged by this app** — there is no Instacart partnership, and the store, delivery window and payment are all the owner's own, inside Instacart. Verified against real Postgres: inventory inherits the contractor-book scoping through its `properties` subquery, so a contractor sees the cupboard of a home in their own book or one they're assigned to clean and nothing else; the studio owner sees her whole organization; a client sees only their own home and can record a reorder for it but not for anyone else's. |
